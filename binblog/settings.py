import os
from binblog.secret_key import SECRET_KEY
...
MIDDLEWARE = [
    ...
    'django.contrib.admindocs.middleware.XViewMiddleware',  # admindoc书签中间件
    'blog.middleware.LoadTimeMiddleware'  # 页面加载时间中间件
]


DJANGO_DEBUG_TOOLBAR = True  # 启用debug-toolbar，在开发场景下使用

if DJANGO_DEBUG_TOOLBAR:
    INTERNAL_IPS = ['127.0.0.1']
    INSTALLED_APPS += ['debug_toolbar', ]
    MIDDLEWARE += ['debug_toolbar.middleware.DebugToolbarMiddleware', ]
    DEBUG_TOOLBAR_PANELS = [  # debug插件
        ...
        # 'debug_toolbar.panels.redirects.RedirectsPanel', # 每次重定向都要提醒很烦
        'debug_toolbar.panels.profiling.ProfilingPanel',
    ]
    DEBUG_TOOLBAR_CONFIG = {
        'DISABLE_PANELS': [
            'debug_toolbar.panels.redircts.RedirectsPanel',  # 意为不适用调试面板上的哪个功能
        ],
        'SHOW_TEMPLATE_CNTEXT': True,  # 允许查看templates的源html数据
        'SQL_WARNING_THRESHOLD': 0.5,  # 数据库延时的提示，单位是ms，不设置默认情况下是500ms
        'SHOW_COLLAPSED': True,  # 设置debugtollbar为下拉框模式
    }
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            'context_processors': [
                ...
                'blog.context_processors.context_setting',  # 自定义模板全局变量, 网站常量
            ],
        },
    },
]
...
DATABASES = {
    'default': {
        ...
        'NAME': 't_blog',  # 数据库名称, 需要自己手动创建, 编码指定为utf8
        'USER': 'root',  # 数据库账号
        'PASSWORD': '',  # 数据库密码
...
CACHES = {
    'default': {
        'BACKEND': 'django.core.cache.backends.locmem.LocMemCache',  # 设置为本地内存缓存
        'TIMEOUT': 60 * 60 * 1,  # 过期时间： 一个小时
        'LOCATION': 'unique-snowflake',  # 根据雪花算法设置唯一的序号
    }
}

# logging的配置 https://docs.djangoproject.com/en/2.2/topics/logging/
SERVER_EMAIL = EMAIL_HOST_USER  # 发送错误信息时的邮箱
ADMINS = [('admin', '941811450@qq.com')]  # 管理员元组列表, 500错误邮件通知
BASE_LOG_DIR = os.path.join(BASE_DIR, 'log')  # 日志存放的目录, 需要手动创建
LOG_PREFIX = 'blog-'  # 日志文件命名前缀
