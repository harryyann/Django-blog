...
from xadmin.templatetags.xadmin_tags import register
from xadmin.views import CommAdminView, BaseAdminView
from blog.models import Article, Setting, Category, Comment, Tag, SideBar, Photo, GuestBook, Link
...
class ArticleAdmin(object):
    ...
    list_export_fields = ('id', 'title', 'author', 'category_link', 'views')

    refresh_times = (3, 5)

    show_detail_fields = ['id', 'title', 'author', 'category_link', 'views']
    list_editable += ('title', 'author', 'category_link')
...
class GlobalSettings(object):
    """xadmin的全局配置"""
    site_title = "个人博客后台管理"  # 设置站点标题
    site_footer = "Horace's personal blog 2020"  # 设置站点的页脚
    menu_style = "accordion"  # 设置菜单折叠


xadmin.site.register(BaseAdminView, BaseSetting)
xadmin.site.register(CommAdminView, GlobalSettings)
xadmin.site.register(Setting, SettingAdmin)  # 站点配置
xadmin.site.register(Category, CategoryAdmin)  # 文章分类
xadmin.site.register(Article, ArticleAdmin)  # 文章
xadmin.site.register(Comment, CommentAdmin)  # 文章评论
xadmin.site.register(Tag, TagAdmin)  # 文章标签
xadmin.site.register(SideBar, SideBarAdmin)  # 侧边栏
xadmin.site.register(Photo, PhotoAdmin)  # 相册
xadmin.site.register(GuestBook, GuestBookAdmin)  # 留言板
xadmin.site.register(Link, LinkAdmin)  # 友情链接
